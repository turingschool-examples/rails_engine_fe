...
RSpec.describe 'Merchants Facade' do
  ...
  it 'gets a list of merchant items' do
    ...
    expect(response.description).to eq('Sunt eum id eius magni consequuntur delectus veritatis. Quisquam laborum illo ut ab. Ducimus in est id voluptas autem.')
    expect(response.unit_price).to eq(42.91)
    expect(response.merchant_id).to eq(1)
  end
